...
export default defineEventHandler(async (event) => {
  ...
  const pending = findLatestUnresolvedMessage(conversation.id, 'order_confirmation')
  if (!pending) {
    conflict('Aucune demande de confirmation en attente pour cette conversation.')
  }

  resolveMessage(conversation.id, pending.id)
  const confirmation = addSystemMessage(conversation.id, 'Le prestataire a confirmé la prise en charge de la commande.')
  const locationRequest = addSystemMessage(
    conversation.id,
