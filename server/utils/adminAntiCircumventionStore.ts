...
import { listContournementAttempts } from '~~/server/utils/contournementAttemptStore'
import { toUser } from '~~/server/utils/userStore'

/**
 * Anti-désintermédiation (#dashboard-admin, module 9) — deux signaux réels :
 ...
 * server/utils/adminAntiCircumventionDemo.ts, clairement marqué TODO.
 */
...
export async function listContournementSignals(): Promise<ContournementSignal[]> {
  const attempts = await listContournementAttempts()
...
export async function markFalsePositive(userId: string, markedBy: string, note?: string): Promise<void> {
  await prisma.riskFalsePositive.upsert({
    where: { userId },
    create: { userId, markedBy, note },
    update: { markedBy, note, markedAt: new Date() },
  })
}
