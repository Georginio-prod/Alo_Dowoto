import { getFaqCategoriesFr } from '../../data/faq'
import { getProviderDetail, searchProviders, searchProvidersNearby } from '../providerDirectoryService'
import type { AiToolDefinition } from './types'

/**
 * Outils réels que l'assistant peut appeler (#geoloc, 2.2), portés iso depuis
 * `server/utils/ai/tools.ts` (ADR-0016). Règle anti-hallucination : toute
 * recommandation passe par ces outils, jamais inventée. Chaque outil retourne
 * des données réelles (annuaire, FAQ) — aucune fiche fictive générée ici.
 */
...
/** Score par mots communs (>2 caractères) entre la question et chaque entrée FAQ. */
...
/** Exécute un outil par son nom (#geoloc, 2.2) — dispatcher unique. Iso Nitro. */
