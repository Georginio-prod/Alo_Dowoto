...
import { sendAdminMessage } from './adminMessagingService'

/**
 * Médiation admin des litiges escrow (#dashboard-admin, module 6), portée iso
 * depuis `server/utils/adminDisputeStore.ts` (ADR-0017) — l'équipe WorkTogo
 * tranche explicitement (y compris avec partage du montant), en complément du
 * dénouement automatique. Réutilise les primitives escrow/portefeuille ; chaque
 * décision est tracée par la route appelante.
 */
 ...
 * `client` rembourse l'intégralité (sans commission) ; `split` partage selon
 * `providerSharePercent` (0-100), sans commission — arbitrage manuel documenté,
 * distinct du calcul automatique. Iso `adminResolveDispute`.
 */
export async function adminResolveDispute(
  ...
  if (outcome === 'provider') {
    ...
    await prisma.$transaction(async (tx) => {
      await walletMovementRepository.credit({ walletUserId: row.clientId, type: 'escrow_refund', amount: row.amount, reference: row.id, counterpartyUserId: row.providerId }, tx)
    ...
    await prisma.$transaction(async (tx) => {
      if (providerShare > 0) await walletMovementRepository.credit({ walletUserId: row.providerId, type: 'escrow_release', amount: providerShare, reference: row.id, counterpartyUserId: row.clientId }, tx)
      if (clientShare > 0) await walletMovementRepository.credit({ walletUserId: row.clientId, type: 'escrow_refund', amount: clientShare, reference: row.id, counterpartyUserId: row.providerId }, tx)
...
/** Demande une preuve complémentaire aux deux parties (notification in-app). */
