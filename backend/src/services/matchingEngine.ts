/**
 * Moteur de scoring pondéré des prestataires (#54/#187), porté **verbatim**
 * depuis `server/utils/matchingEngine.ts` (ADR-0016). Fonctions **pures**
 * (aucune dépendance à un store ou au transport HTTP) : classement pour une
 * demande client (`rankProviders`) et mise en avant sur l'accueil chercheur
...
/**
 * Moyenne bayésienne : ramène la note d'un prestataire avec peu d'avis vers une
 * moyenne de référence (`priorMean`), pondérée par un nombre d'avis fictif
 * (`priorWeight`). Empêche qu'un seul avis à 5★ écrase des prestataires établis
 * avec des dizaines d'avis solides (#187).
 */
