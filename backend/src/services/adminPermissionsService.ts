...
/**
 * Permissions granulaires des comptes administrateurs (#admin), portées iso
 * depuis `server/utils/adminPermissions.ts` (ADR-0016). Super-administrateur =
 * `adminPermissions` NULL/vide en base → `null` ici = « toutes les permissions »
 * (rétrocompatibilité : les admins créés avant cette fonctionnalité gardent
 * l'accès total). Les autres admins portent un tableau JSON de clés.
 */
...
/** Décode la valeur JSON stockée. `null` = super-administrateur (toutes les permissions). */
...
export async function getAdminPermissions(userId: string, users: UserRepository = userRepository): Promise<AdminPermission[] | null> {
  const user = await users.findById(userId)
  return parsePermissions(user?.adminPermissions ?? null)
}
