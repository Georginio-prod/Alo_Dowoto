...
/**
 * Agrégats de la Vue d'ensemble (#dashboard-admin, module 1), portés iso depuis
 * `server/utils/adminOverview.ts` (ADR-0016) — calculés à la volée à partir des
 * tables Prisma réelles (EscrowOrder, WalletMovement, Subscription, User,
 * Conversation, Review). La note moyenne agrège tous les avis mutuels
 * (`prisma.review`, persisté ADR-0015) là où Nitro lisait `reviewStore`.
 *
 * Seul le tout premier palier de l'entonnoir de conversion (« recherche ») n'a
 * aucune source de données et reste estimé — voir `ESTIMATED_SEARCH_TO_REQUEST_RATIO`.
 */
...
export interface ConversionFunnel {
  ...
  /** `true` : `searches` est une estimation (pas de suivi de consultation de fiche). */
  searchesEstimated: boolean
}

/**
 * Ratio appliqué à `requestsSent` pour estimer `searches` tant qu'aucune
 * consultation de fiche n'est tracée. Valeur pragmatique — à remplacer dès
 * qu'un vrai suivi existera. Iso Nitro.
 */
...
export async function getRecentActivity(limit = 10): Promise<RecentActivityEntry[]> {
  ...
  const entries: RecentActivityEntry[] = [
    ...
    ...disputes.map((d): RecentActivityEntry => ({
      ...
      // `disputedAt` est garanti non-null par le `where` de la requête, mais Prisma le type toujours nullable.
