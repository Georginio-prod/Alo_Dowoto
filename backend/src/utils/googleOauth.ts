 ...
 * Portée depuis `server/utils/googleAuth.ts` (ADR-0016) : mêmes cookies, même
 * flux « authorization code », mêmes endpoints. Seule adaptation runtime : les
 * appels réseau passent par le `fetch` global de Node (Nitro utilisait `$fetch`).
 *
 ...
 * .env.example). Sans configuration, le parcours renvoie une erreur explicite
 * (`/auth?error=google_config`) plutôt que d'échouer en silence.
 */
...
export async function fetchGoogleProfile(
  ...
  try {
    ...
    const infoResponse = await fetch(GOOGLE_USERINFO_ENDPOINT, {
      headers: { Authorization: `Bearer ${tokens.access_token}` },
    })
    if (!infoResponse.ok) return null
    const info = (await infoResponse.json()) as GoogleUserInfoResponse
