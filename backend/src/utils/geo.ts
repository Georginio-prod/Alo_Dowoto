/**
 * Calcul de distance entre deux points GPS (#263), porté **verbatim** depuis
 * `server/utils/geo.ts` (ADR-0016). Formule de Haversine — précision suffisante
 * à l'échelle d'une recherche par ville/quartier, aucune dépendance externe.
 */
 ...
 * utiliser en pré-filtrage avant le calcul Haversine exact. Approximation
 * volontairement large (1° de latitude ≈ 111 km, corrigée par le cosinus de la
 * latitude pour la longitude) : elle ne doit jamais exclure un point réellement
 * dans le rayon, seulement réduire le nombre de candidats à vérifier précisément.
 */
 ...
 * prestataire en position approximative ne doit jamais exposer son point exact
 * (domicile) dans les réponses API. Arrondi à 2 décimales ≈ quelques centaines
 * de mètres. Décalage déterministe (pas aléatoire) : stable d'une requête à
 * l'autre (pas de « saut » du marqueur au fil des rafraîchissements).
 */
