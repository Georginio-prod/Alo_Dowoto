 ...
 * prestataire) ainsi qu'un accès Google (visuel — non fonctionnel, voir
 * `chooseGoogle` ci-dessous : aucune intégration OAuth réelle disponible).
 */
...
const props = defineProps<{ cta: string; mode: 'login' | 'signup' }>()
const emit = defineEmits<{
  sent: [payload: { method: Method; value: string; devCode?: string; profile?: SignupProfile }]
}>()
...
function chooseGoogle() {
  // Aucune intégration OAuth Google réelle (identifiants/app non disponibles) :
  // le bouton reste visible pour la parité visuelle avec la maquette, mais
  // affiche explicitement son indisponibilité plutôt que de simuler une
  // connexion qui n'existe pas.
  googleNotice.value = "L'inscription avec Google n'est pas encore disponible. Utilisez le formulaire ci-dessous."
}
...
<template>
  <div>
    <template v-if="isSignup">
      ...
      <p v-if="googleNotice" class="mb-3.5 text-center text-[12px] text-muted">{{ googleNotice }}</p>
