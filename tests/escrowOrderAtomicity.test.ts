...
import {
  recordEscrowOrderCheckIn,
  recordEscrowOrderCheckOut,
} from "~~/server/utils/escrowInterventionProof";
import {
...
describe("Atomicité des flux séquestre (#366) — libération", () => {
  it.skip("ne crédite prestataire et plateforme qu’une seule fois même si la libération est rejouée sur un état périmé", async () => {
    const client = id();
    const provider = id();
    const amount = 3000;
    ...
    const deliveredOrder = await payAndDeliver(
      conversation.id,
      client,
      provider,
      amount,
    ...
    expect(await getBalance(PLATFORM_WALLET_USER_ID)).toBe(
      platformBefore + commission,
    );
  });
